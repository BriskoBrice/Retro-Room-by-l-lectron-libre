(() => {
  ...
  const FOLDER_COMPACT=new Map();
  for(const [name,id] of FOLDER_MAP) FOLDER_COMPACT.set(name.replace(/\s+/g,''),id);
  ...
  function folderSystem(file){
    ...
    const seg=rel.split('/').slice(0,-1).map(normalizeToken);
    for(let i=seg.length-1;i>=0;i--){
      const token=seg[i];
      const hit=FOLDER_MAP.get(token)||FOLDER_COMPACT.get(token.replace(/\s+/g,''));
      if(hit)return hit;
    }
    return null;
  }
