function launchRom(file){
  ...
  if(window.BiosSupport?.requiresBios(systemId)){
    const ready=window.BiosSupport.ensureBios(systemId,file,()=>launchRom(file));
    if(!ready)return;
  }
  const core=sys.core;
  ...
  window.EJS_player='#game';window.EJS_core=core;window.EJS_controlScheme=sys.controlScheme||core;window.EJS_VirtualGamepadSettings=[];window.EJS_gameUrl=arcadeFileMode?file:romObjectUrl;window.EJS_gameName=cleanName(file.name);window.EJS_biosUrl=window.BiosSupport?.biosFor(systemId)||'';window.EJS_pathtodata=EJS_DATA;window.EJS_startOnLoaded=false;window.EJS_startButtonName='LANCER LE JEU';window.EJS_alignStartButton='center';window.EJS_language='fr-FR';window.EJS_disableAutoLang=true;window.EJS_browserMode='desktop';window.EJS_threads=!!sys.threads;window.EJS_noAutoFocus=false;window.EJS_backgroundColor='#020504';window.EJS_color='#67e995';window.EJS_volume=.75;window.EJS_Buttons=minimalButtons();
  ...
  window.EJS_onGameStart=function(){emulatorStarted=true;emulatorReady=true;clearTimeout(watchdog);bootScreen.style.display='none';startEmulatorUiCleaner();romBtn.textContent='NEW';if(window.BiosSupport?.requiresBios(systemId)){setStatus('MOTEUR '+systemLabel(systemId)+' ACTIF');showToast('Moteur '+systemLabel(systemId)+' démarré — chargement du jeu…');}else{setStatus('PLAY — '+systemLabel(systemId));showToast(systemLabel(systemId)+' lancée');}};
